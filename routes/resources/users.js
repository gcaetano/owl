...
router.get('/', (req, res, next) => {
    ...
    BL_Users.read(function (err, data) {
...
router.get('/:id',(req, res, next) => {
    ...
    if(req.params && req.params.id){
        BL_Users.readOne(req.params.id, function (err, data) {
            var result = {success: !err, data: data || []};
            res.status(200).send(result);
            res.end();
        })
...
router.post('/',(req, res, next) => {
    ...
    if(req.body && req.body !== {}){
        BL_Users.create(req.body, req.session.user, (err, insert) => {
...
router.put('/:id', function (req, res) {
    ...
    if(req.body){
        BL_Users.update(req.params.id, req.body, req.session.user, function (err, update) {
...
router.delete('/:id', function (req, res) {
    ...
    if(req.params && req.params.id){
        BL_Users.delete(req.params.id, function (err, remove) {

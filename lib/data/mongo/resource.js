...
Resource.prototype.addToSet = function(sessionID, collection, id, modifier, callback) {
    
    Database.getOwlConnection((db) => {
        var coll = db.collection(collection);
        var query = {_id: new ObjectID(id)};
        var set = { $addToSet: modifier };
        logger.info("DTA [%s] data/mongo/%s.update[$addToSet] %j", sessionID, collection, set);
        
        coll.update(query, set, (err, update) => {
            if (err) {
                logger.error("DTA [%s] data/mongo/%s.update[$addToSet] %j", sessionID, collection, err.errmsg);
                callback(err, null);
            } else {
                logger.info("DTA [%s] data/mongo/%s.update[$addToSet] %j ", sessionID, collection, update);
                callback(null, update);
            }
        });
    });
...
Resource.prototype.pull = function(sessionID, collection, idGroup, modifier, callback) {
    
    Database.getOwlConnection((db) => {
        var coll = db.collection(collection);
        var query = {_id: new ObjectID(idGroup)};
        var pull = { $pull: modifier };
        logger.info("DTA [%s] data/mongo/%s.update [query :: %j]  [$pull :: %j]", sessionID, collection, query, pull);
        
        coll.update(query, pull, (err, update) => {
            if (err) {
                logger.error("DTA [%s] data/mongo/%s.update[$pull] %j", sessionID, collection, err.errmsg);
                callback(err, null);
            } else {
                logger.info("DTA [%s] data/mongo/%s.update[$pull] %j ", sessionID, collection, update);
                callback(null, update);
            }
        });
    });

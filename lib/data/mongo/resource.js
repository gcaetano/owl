...
Resource.prototype.read = function(collection, query, callback) {
    Database.getOwlConnection(function (db) {
        ...
        coll.find(query).toArray(function (err, result) {
            if (err) {
                ...
                logger.info("DTA | data/mongo/%s.get query %j records %d ", collection, query, result.length);
...
Resource.prototype.create = function(collection, representation, callback) {
    Database.getOwlConnection((db) => {
        var coll = db.collection(collection);
        coll.insert(representation, (err, insert) => {
            if (err) {

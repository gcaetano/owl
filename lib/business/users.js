...
BL_Users.prototype.create = function(req, object, callback){
    bcrypt.genSalt(saltRounds, (err, salt) => {
        bcrypt.hash(myPlaintextPassword, salt, (err, hash) => {
            ...
            object.created = { date: new Date(), issuer: req.session.user};
...
BL_Users.prototype.auth =(req, username, password, callback) =>{
    var query = {username: username};
    read(req.sessionID, query, (err, data) => {
        if(!err){
            var user = _.first(data);
            logger.info("BSN [%s] business/%s.auth %s %j", req.sessionID, collection, user.username, user);   

...
function fillGroups(req, groups, callback){
        ...
        //            var vid = records[i].vid;
        //            var from = new Date(records[i].tmx).getTime();
        //            redis.getHistoricFrom(vid, from, function (reply) {
        //                if (reply) result = result.concat(reply);
        //                callback();
        //            });
        //        }
        //    })(i));
    ...
    async.each(groups,
        function(group, callback){
            fillUser(req, group.users, function(err, data){
                if(!err) group.users = data;
                callback();
            })
        },

...
BL_Groups.prototype.create = function (req, object, callback) {
    ...
    Resource.create(req.sessionID, collection, object, function (err, insert) {
        if (!err) insert.result.id = insert.insertedIds[0];
    ...
    for (let i = 0; i < groups.length; i++) {
        asyncTasks.push((function (i) {
            return function (callback) {
                var group = groups[i];
                getUsersFromGroup(req, group, function (err, data) {
                    if (!err) {
                        groups[i].users = data;
                    }
                    callback();
...
function getUsersFromGroup(req, group, callback) {
    BL_Users = require('./users').BL_Users;
    var options = req.query ? Helper.getReadOptions(req.query) : {};
    var query = { group: group._id.toString() };
    BL_Users.findBy(req, query, options, function (err, result) {
        callback(err, result);
    })
}

// it's will fill each user information...
function fillUser(req, group, callback) {
    BL_Users = require('./users').BL_Users,
        users = [];
    async.each(_users,
